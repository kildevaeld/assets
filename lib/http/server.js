...
var AssetsRouter = function () {
    ...
    _createClass(AssetsRouter, [{
        ...
        value: function middleware(req, res, next) {
            ...
            return this[route.fn].call(this, req, res, match.length == 2 ? match[1] : undefined).catch(function (e) {
        ...
        value: function list(req, res) {
            return __awaiter(this, void 0, Promise, function* () {
                ...
                if (query.id) {
                    var _asset = yield this._assets.getById(query.id);
                    if (!_asset) {
                        throw new HttpError("Not Found", 400);
                    }
                    if (req.method === 'DELETE') {
                        yield this._assets.remove(_asset);
                        return yield this._writeJSON(res, {
                            status: 'ok'
                        });
                    }
                    return yield this._writeJSON(res, _asset);
                }
                ...
                if (query.q) {} else {
                    ...
                    var offset = Math.ceil(count / limit) * page;
                    if (offset > count) {
                        result = [];
                    } else {
...
var asset = new index_1.Assets({
    metaStore: 'file',
    dataStore: 'file'
});
asset.initialize().then(function () {
    var router = new AssetsRouter(asset);
